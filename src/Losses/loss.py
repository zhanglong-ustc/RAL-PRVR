...
from Models.RAL4gmmv2.model_components import clip_nce, frame_nce
...
class query_diverse_loss(nn.Module):
    ...
    def qdl_v1(self, x, label_dict):

        "The earliest published version of qdl_v1 only contains logs, without the factors preceding the logs."
        ...
        # When calculating the N_sim_sum matrix, use torch.where to select the neg_exp elements at the positions where N_one_hot is 1, 
        # set the elements at all other positions to 0, and then sum along dimension 0 to obtain the N_sim_sum vector.
...
class loss(nn.Module):
    ...
    def forward(self, input_list, batch):
        ...
        query = input_list[5]
        ot_loss = input_list[6]

        MIL_loss = 4e-3 * input_list[7]  # 0.2312
        vib_loss = 2.5e-4 * input_list[8]  # 0.09
        # con_loss = 3e-3 * input_list[9] # 0.03
        ...
        # qdl_loss = self.cfg['loss_factor'][2] * self.qdl.qdl_v1(query, label_dict)
        
        loss = clip_nce_loss + frame_nce_loss + clip_trip_loss+ frame_trip_loss + qdl_loss + MIL_loss + vib_loss + self.cfg['loss_factor'][3] * ot_loss
        ...
        return loss, {"loss_overall": float(loss), 'clip_nce_loss': clip_nce_loss,
                      ...
                      'MiL_loss': MIL_loss, 'Vib_loss': vib_loss}

    def get_clip_triplet_loss(self, query_context_scores, labels):
        '''
        query_context_scores: [Nq, Nv], similarity score matrix
        labels: length is Nq, the video indexs for each query
        '''
        v2t_scores = query_context_scores.t() #[Nv, Nq]
        t2v_scores = query_context_scores #[Nq, Nv]
        labels = np.array(labels)

        # cal_v2t_loss
        v2t_loss = 0
        for i in range(v2t_scores.shape[0]):
            pos_pair_scores = torch.mean(v2t_scores[i][np.where(labels == i)]) #mean(Mq_for_v) mean of the positive scores for i-th video
            neg_pair_scores, _ = torch.sort(v2t_scores[i][np.where(labels != i)[0]], descending=True) #[Nq-Mq_for_v]
            if self.cfg['use_hard_negative']:
                ...
                sample_neg_pair_scores = neg_pair_scores[
                    torch.randint(0, v2t_sample_max_idx, size=(1,)).to(v2t_scores.device)] #[1] random select a negative query
