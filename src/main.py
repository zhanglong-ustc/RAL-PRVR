...
def train_one_epoch(epoch, train_loader, model, criterion, cfg, optimizer):
    ...
    loss_meters = OrderedDict(clip_nce_loss=AverageMeter(), clip_trip_loss=AverageMeter(),
                      frame_nce_loss=AverageMeter(), frame_trip_loss=AverageMeter(),
                              loss_overall=AverageMeter(), MiL_loss =AverageMeter(), Vib_loss = AverageMeter())
    ...
    to_write = train_log_txt_formatter.format(time_str=time.strftime("%Y_%m_%d_%H_%M_%S"), epoch=epoch,
