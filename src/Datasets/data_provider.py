...
def collate_train(data):
    ...
    clip_video_features, frame_video_features, captions, idxs, cap_ids, video_ids, max_ctx_len = zip(*data)
    ...
    frame_videos = torch.zeros(len(frame_video_features), max(max_ctx_len), frame_vec_len)
    videos_mask = torch.zeros(len(frame_video_features), max(max_ctx_len))
...
def collate_frame_val(data):
    clip_video_features, frame_video_features, idxs, video_ids, max_ctx_len = zip(*data)
...
class Dataset4PRVR(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return clip_video_feature, frame_video_feature, cap_tensors, index, cap_ids, video_id, max_ctx_len
...
class VisDataSet4PRVR(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return clip_video_feature, frame_video_feature, index, video_id, max_ctx_len
