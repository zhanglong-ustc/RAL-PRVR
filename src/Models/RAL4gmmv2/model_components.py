...
class clip_nce(nn.Module):
    ...
    def forward(self, labels, label_dict, q2ctx_scores=None, contexts=None, queries=None):
        ...
        label_dict: length is Nv, the query indexs for each video
        q2ctx_scores: [Nq, Nv], similarity score matrix
        logsumexp(x)_i = log \sum_j exp(x_{ij})
        '''
        ...
        t2v_nominator = q2ctx_scores[diagnoal, labels]  #[Nq] similarity scores of the Nq GT query-video pairs
        t2v_nominator = torch.logsumexp(t2v_nominator.unsqueeze(1), dim=1) #logsumexp([Nq,1])  [Nq]
        t2v_denominator = torch.logsumexp(q2ctx_scores, dim=1) #logsumexp([Nq, Nv])  [Nq]
        
        # For v2t retrieval (one-to-many)
        v2t_nominator = torch.zeros(vid_bsz).to(q2ctx_scores) #[Nv]
        v2t_denominator = torch.zeros(vid_bsz).to(q2ctx_scores) #[Nv]
        for i, label in label_dict.items():
            v2t_nominator[i] = torch.logsumexp(q2ctx_scores[label, i], dim=0) #logsumexp([Mq_for_v]) logsumexp(similarity scores of the Mq_for_v pairs for i-th video)
            v2t_denominator[i] = torch.logsumexp(q2ctx_scores[:, i], dim=0) #logsumexp([Nq]) 
        # ipdb.set_trace()
        if self.reduction:
